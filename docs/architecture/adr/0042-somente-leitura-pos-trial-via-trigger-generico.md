...
`enforce_capacidade_obras`/`enforce_email_confirmado` (SPEC 098 Fase 0/1), que cobrem só 2
tabelas cada. O soft delete (SPEC 060, `supabase/migrations/20260859000000_soft_delete_via_rpc.sql`)
...
- **Opção A, replicar o padrão de trigger por tabela** (como capacidade). Prós: mesmo
  ...
  igual, e cada tabela nova no produto (o schema já tem ~180 migrations, isso acontece com
  frequência) exige lembrar de adicionar o trigger. Esquecer um = furo silencioso.
...
- Tabela nova migrada depois desta: se tiver `empresa_id`, entra automaticamente no
  ...
  tabela; caso esqueçam, o pior caso é uma tabela nova SEM o trigger até o próximo
...
leitura_desde !== null`, renderiza `<Layout />` normalmente (navega, exporta) com um
  ...
  de criar/editar/excluir desabilitados na UI ficaram fora do escopo desta entrega
  (redundante com o servidor); é um risco aceito, não um esquecimento (ver nota de
  implementação abaixo).

## Nota de implementação (09/09, PR #487)

- Migration `20260936000000_retencao_pos_trial_fase3.sql`. 28 pgTAP novos cobrindo o
  trigger em tabelas representativas de financeiro (`despesas`, `receitas`), `projetos` e
  `obras`, confirmando que `INSERT`/`UPDATE`/`DELETE` são bloqueados, `SELECT` continua
  liberado, `service_role` passa direto e as tabelas da allowlist (`notificacoes`,
  `pilar_subscriptions`) nunca ganham o trigger.
- Achado durante a implementação da ADR 0043 (exclusão de empresa): a RPC
  `excluir_empresa_retencao()`, quando chamada manualmente por um ultra-admin autenticado
  (não pelo cron via `service_role`), roda dentro de uma sessão cujo `auth.role()` continua
  `'authenticated'` mesmo sendo `SECURITY DEFINER`; a empresa alvo normalmente já está em
  leitura, então o próprio trigger criado por este ADR bloquearia a exclusão. Fix: o
  trigger ganhou uma segunda condição de bypass, `current_setting('pilar.retencao_em_curso',
true) = 'on'`, uma GUC local à transação que só `excluir_empresa_retencao()` liga.
  Detalhe completo no ADR 0043.
- `formulários e botões desabilitados na UI` (redundância client-side citada acima) não
  entrou nesta PR: o banner informa o estado e o servidor já bloqueia de verdade, então o
  pior caso sem essa redundância é o usuário ver o erro genérico `P0001 empresa_em_leitura`
  ao tentar editar, não um furo de segurança. Fica como melhoria de UX pendente, não como
  gap deste ADR.
...
- [ADR 0043](./0043-exclusao-de-empresa-anonimiza-financeiro-apaga-o-resto.md): a RPC de
  exclusão precisa driblar este mesmo trigger na empresa que ela está apagando.
