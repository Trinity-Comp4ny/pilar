...
7. **Novo (08-17): eixo "obras ativas" não tem NENHUMA wiring de billing.** Confirmado por busca: `pilar_subscription_plans` não tem coluna `max_obras`; nenhuma RPC ou hook conta "obras ativas" por empresa; a tabela `obras` (`20260730165000_obras_mvp.sql`) tem `status` (`planejada`/`em_andamento`/`paralisada`/`concluida`) e `deleted_at`, o que dá a matéria-prima pra um contador de "obra ativa" (mesmo padrão de "projeto ativo"), mas ninguém lê isso hoje pra aplicar limite de plano. Além disso, Obras continua sendo **toggle por empresa** (`empresas.features.obras`), não **entitlement por plano**, ligado só pra VRZ. Vender "2/5/12 obras por plano" exige: (a) coluna `max_obras` (ou JSON de limites) em `pilar_subscription_plans`, (b) contador de obras ativas por empresa, (c) o mesmo elo plano→features do item 2, agora cobrindo obra além de projeto. É o MESMO esforço pendente desde a v2, duplicado pro segundo eixo: não é trabalho novo em categoria, é o mesmo gap, maior escopo.
