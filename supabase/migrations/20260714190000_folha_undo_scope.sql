-- Refina fechar_folha_agente: registra os IDs das linhas criadas em agent_runs.result.linhas_ids,
-- para o "Desfazer" apagar EXATAMENTE essas linhas (não todas as pendentes do mês — evita dano colateral).
...
BEGIN
  ...
  END IF;
  IF v_run.agent_type != 'fechar_folha' THEN RAISE EXCEPTION 'Tipo de agente não suportado: %', v_run.agent_type; END IF;

  v_mes := NULLIF(v_run.result->>'mes', '')::int;
  v_ano := NULLIF(v_run.result->>'ano', '')::int;
  IF v_mes IS NULL OR v_mes < 1 OR v_mes > 12 OR v_ano IS NULL THEN RAISE EXCEPTION 'Mês/ano inválidos'; END IF;
  ...
  WITH ins AS (
    ...
    FROM jsonb_to_recordset(v_run.result->'linhas') AS l(
      pessoa_id uuid, salario_fixo numeric, total_area_projetada numeric,
      valor_m2 numeric, adicional_variavel numeric, total_receber numeric
    )
    WHERE l.pessoa_id IS NOT NULL
    RETURNING id
  )
  SELECT array_agg(id), count(*) INTO v_ids, v_count FROM ins;

  IF v_count = 0 THEN RAISE EXCEPTION 'Nenhuma linha de folha para inserir'; END IF;

  UPDATE public.agent_runs
    SET status='executed', entity_type='folha',
        result = result || jsonb_build_object('linhas_ids', to_jsonb(v_ids)),
