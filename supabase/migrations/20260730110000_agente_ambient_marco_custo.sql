-- Agente ambient (spec 007, Fase 3): +2 regras de achado (spec do workspace).
--   - marco_proximo    : marco de faturamento a vencer (liga prazo a dinheiro a entrar)
--   - custo_nao_lancado: projetos ativos com contrato mas SEM despesa lançada. NÃO é
--                        margem (que depende de dado dormente, spec 004) — é COMPLETUDE:
--                        conta o que falta preencher para o lucro ser calculável. Copy
--                        honesta: "lucro não calculável", nunca "margem baixa".
-- Reescreve gerar_alertas_ambient() por inteiro (CREATE OR REPLACE exige o corpo todo).

-- Amplia os tipos permitidos (marco_proximo já existia; custo_nao_lancado é novo).
...
ALTER TABLE public.alertas
  ADD CONSTRAINT alertas_tipo_check CHECK (
    tipo = ANY (ARRAY[
      ...
      'prazo_estourado', 'disciplina_atrasada', 'custo_nao_lancado'
    ])
  );
...
BEGIN
  ...
  -- Disciplinas de projeto atrasadas.
  INSERT INTO public.alertas (empresa_id, tipo, severidade, titulo, mensagem, referencia_tipo, referencia_id)
  ...
  -- NOVO: marcos de faturamento a vencer nos próximos 7 dias (dinheiro a entrar).
  INSERT INTO public.alertas (empresa_id, tipo, severidade, titulo, mensagem, referencia_tipo, referencia_id)
  ...
  -- NOVO: projetos ativos com contrato mas SEM despesa lançada (lucro não calculável).
  -- Agregado por empresa (referencia_id nulo). Move a métrica "% de projetos com margem
  -- calculável" sem afirmar nenhum R$ de lucro — só aponta o que falta preencher.
  INSERT INTO public.alertas (empresa_id, tipo, severidade, titulo, mensagem, referencia_tipo, referencia_id)
