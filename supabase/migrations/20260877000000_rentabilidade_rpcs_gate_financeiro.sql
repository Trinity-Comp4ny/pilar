-- SPEC 073 / ADR 0034: fase 8, achado mais grave do teste ponta a ponta.
--
-- rpc_dashboard_rentabilidade, rpc_projeto_rentabilidade e
-- get_projeto_rentabilidade_detalhe são SECURITY DEFINER (bypassam RLS de
-- projetos/receitas/despesas/marcos_faturamento/projeto_orcamento_fases por
-- completo) e devolvem valor_contrato, margem, receitas, despesas e
-- faturamento de TODOS os projetos da empresa num JSON só, checando apenas
-- empresa_id. Nenhuma das três nunca chamou user_has_feature('financeiro',
-- ...) nem qualquer helper de sigilo — não é regressão desta spec, é um
-- buraco que já existia (a mais antiga é de 000_base_schema.sql), mas é
-- estruturalmente o mesmo furo que motivou o cliente a reclamar, só que
-- devolvendo o relatório inteiro em vez de uma tabela por vez.
--
-- Também tinham GRANT ALL para `anon` (inofensivo na prática porque
-- get_user_empresa_id() é NULL sem sessão, mas fora do padrão do resto do
-- código, que revoga anon explicitamente nas funções financeiras).
--
-- Gate cheio (can_view_financeiro()) nas três: são relatório de dinheiro
-- puro, sem uso legítimo fora de financeiro.
--
-- Corpo reproduzido a partir de 20260869000000_reconecta_rentabilidade_horas_reais
-- (mergeou em staging depois desta branch nascer, mesma numeração de timestamp
-- — daí a renumeração desta leva de migrations): horas_consumidas/custo_mao_de_obra/
-- custo_mo agora vêm de projeto_disciplinas.horas_realizadas/custo_hora, não mais
-- zerados. Só a linha do gate + REVOKE/GRANT muda aqui, resto é idêntico.
...
BEGIN
  IF NOT public.can_view_financeiro() THEN
    RAISE EXCEPTION 'Sem permissão para ver rentabilidade';
  END IF;
...
REVOKE ALL ON FUNCTION public.rpc_dashboard_rentabilidade() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.rpc_dashboard_rentabilidade() TO authenticated;
...
REVOKE ALL ON FUNCTION public.rpc_projeto_rentabilidade(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.rpc_projeto_rentabilidade(uuid) TO authenticated;
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'parcelas', COALESCE((
      ...
      WHERE m.projeto_id = p_projeto_id AND m.deleted_at IS NULL AND m.empresa_id = v_empresa_id
    ), '[]'::jsonb)
  );
END;
$$;

REVOKE ALL ON FUNCTION public.get_projeto_rentabilidade_detalhe(uuid) FROM PUBLIC, anon;
