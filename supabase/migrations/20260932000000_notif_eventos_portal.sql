-- Spec 093 (PR 2): ações do cliente pelo Portal (proposta aceita, entrega aprovada,
-- revisão de entrega solicitada) hoje gravam no banco e não avisam ninguém — o
-- escritório só percebe abrindo a proposta ou a entrega específica. Este bloco avisa
-- o(s) responsável(is) da disciplina/projeto (reusando projeto_disciplina_responsaveis
-- e os helpers _notif_resp_disciplina/_notif_resp_projeto já existentes desde
-- 20260817000100), somado sempre a _notif_gestao_operacional — nunca fica sem
-- destinatário quando não há responsável definido.
--
-- Diferente dos outros blocos desta função (que descrevem um ESTADO que persiste até
-- ser resolvido, ex: pagamento ainda pendente), estes três são EVENTOS pontuais: uma
-- vez aceita, a proposta nunca deixa de estar com status 'aceita'. O dedup padrão de
-- notificar() (não empilha enquanto não lida) não basta aqui, porque, uma vez que a
-- notificação for lida, a mesma condição bateria de novo na próxima varredura do cron
-- e recriaria a notificação para sempre. Por isso cada bloco tem um NOT EXISTS extra
-- contra `notificacoes` (lida ou não), que os outros blocos não precisam.
--
-- gerar_notificacoes_ambient() é reproduzida por inteiro a partir da versão vigente
-- (20260901000000_notif_gestao_operacional_e_roteamento.sql) — mesma prática já
-- documentada lá: CREATE OR REPLACE substitui a função inteira.
...
BEGIN
  ...
  -- ── Obra (spec 090): a obra em si atrasada ──────────────────────────────────────
  FOR rec IN
  ...
  -- ── Guardião de escopo (spec 067/081, FIN-5; roteamento corrigido spec 090) ─────────
  FOR rec IN
  ...
  -- ── Portal do cliente (spec 093, NOVO): proposta aceita, entrega aprovada ou com
  --    revisão solicitada. Evento pontual (não estado que persiste) — o NOT EXISTS
  --    contra `notificacoes` evita recriar a notificação depois de lida, diferente do
  --    dedup padrão de notificar() (que só evita empilhar enquanto não lida). ──────────
  FOR rec IN
    SELECT pr.id, pr.empresa_id, pr.projeto_id, pr.titulo
    FROM public.propostas pr
    WHERE pr.deleted_at IS NULL AND pr.status = 'aceita' AND pr.projeto_id IS NOT NULL
      AND NOT EXISTS (
        SELECT 1 FROM public.notificacoes n
        WHERE n.tipo = 'portal_proposta_aprovada' AND n.referencia_id = pr.id
      )
  LOOP
    v_dest := ARRAY(SELECT DISTINCT unnest(public._notif_gestao_operacional(rec.empresa_id) || public._notif_resp_projeto(rec.projeto_id)));
    v_total := v_total + public.notificar(
      rec.empresa_id, v_dest, 'portal_proposta_aprovada', 'projeto', 'medium',
      'Proposta aprovada pelo cliente: ' || rec.titulo,
      'O cliente aprovou a proposta pelo portal.',
      'proposta', rec.id, '/projetos/' || rec.projeto_id);
  END LOOP;

  FOR rec IN
    SELECT e.id, e.empresa_id, e.projeto_id, e.projeto_disciplina_id, e.titulo
    FROM public.portal_entregas e
    WHERE e.status = 'aprovado' AND e.respondido_em IS NOT NULL
      AND NOT EXISTS (
        SELECT 1 FROM public.notificacoes n
        WHERE n.tipo = 'portal_entrega_aprovada' AND n.referencia_id = e.id
      )
  LOOP
    v_dest := ARRAY(SELECT DISTINCT unnest(
      public._notif_gestao_operacional(rec.empresa_id)
      || CASE WHEN rec.projeto_disciplina_id IS NOT NULL
           THEN public._notif_resp_disciplina(rec.projeto_disciplina_id)
           ELSE public._notif_resp_projeto(rec.projeto_id)
         END
    ));
    v_total := v_total + public.notificar(
      rec.empresa_id, v_dest, 'portal_entrega_aprovada', 'projeto', 'medium',
      'Entrega aprovada pelo cliente: ' || rec.titulo,
      'O cliente aprovou esta entrega pelo portal.',
      'portal_entrega', rec.id, '/projetos/' || rec.projeto_id);
  END LOOP;

  FOR rec IN
    SELECT e.id, e.empresa_id, e.projeto_id, e.projeto_disciplina_id, e.titulo, e.resposta_cliente
    FROM public.portal_entregas e
    WHERE e.status = 'revisao_solicitada' AND e.respondido_em IS NOT NULL
      AND NOT EXISTS (
        SELECT 1 FROM public.notificacoes n
        WHERE n.tipo = 'portal_entrega_revisao_solicitada' AND n.referencia_id = e.id
      )
  LOOP
    v_dest := ARRAY(SELECT DISTINCT unnest(
      public._notif_gestao_operacional(rec.empresa_id)
    ...
    v_total := v_total + public.notificar(
      rec.empresa_id, v_dest, 'portal_entrega_revisao_solicitada', 'projeto', 'high',
      'Cliente pediu revisão: ' || rec.titulo,
      COALESCE(rec.resposta_cliente, 'O cliente solicitou revisão desta entrega pelo portal.'),
      'portal_entrega', rec.id, '/projetos/' || rec.projeto_id);
  END LOOP;
