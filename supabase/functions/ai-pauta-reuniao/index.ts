...
serve(
  withSentry("ai-pauta-reuniao", async (req) => {
    ...
    try {
      ...
      interface ReceitaPendenteRow {
        valor: number;
        descricao: string;
        status: string;
        projeto_id: string;
      }
      interface LeadAtivoRow {
        nome: string;
        valor_estimado: number | null;
        status: string;
        probabilidade: number | null;
      }
      interface ProjetoDetalheRow {
        nome: string;
        status: string;
        valor_contrato: number | null;
      ...
      if (projeto_id) {
        queries.push(adminClient.from("projetos").select("*, clientes(nome)").eq("id", projeto_id).single());
        queries.push(
          adminClient
            .from("timesheets")
      ...
      const projetosAtrasados = projetosAtivos.filter((p) => p.data_final && new Date(p.data_final) < now);
...
${projetosAtivos
  .slice(0, 8)
  .map(
    (p) => `- ${p.nome}: ${p.status}, ${p.percentual_conclusao || 0}% concluído, prioridade ${p.prioridade || "N/A"}`
  )
  .join("\n")}
