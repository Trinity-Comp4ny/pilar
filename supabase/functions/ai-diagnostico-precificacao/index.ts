...
serve(
  withSentry("ai-diagnostico-precificacao", async (req) => {
    ...
    try {
      ...
      const [{ data: projetos }, { data: timesheets }, { data: pessoas }, { data: propostas }] = await Promise.all([
        adminClient
          .from("projetos")
          .select(
            "id, nome, status, valor_contrato, orcamento, area_m2, tipologia, disciplinas, data_inicio, data_final, percentual_conclusao"
          )
      ...
      const margemMedia =
        projetosConcluidos.length > 0
          ? projetosConcluidos.reduce((s: number, p) => s + parseFloat(p.margem_real_pct || "0"), 0) /
            projetosConcluidos.length
          : 0;
      const projetosComValorHora = projetosConcluidos.filter((p) => p.valor_hora_real);
      const valorHoraMedio =
        projetosComValorHora.length > 0
          ? projetosConcluidos.reduce((s: number, p) => s + parseFloat(p.valor_hora_real || "0"), 0) /
            projetosComValorHora.length
          : 0;
