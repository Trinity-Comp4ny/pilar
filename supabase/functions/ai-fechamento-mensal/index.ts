...
serve(async (req) => {
  ...
  try {
    ...
    if (!canProceed) {
      return new Response(
        JSON.stringify({ error: "Limite mensal de consultas IA atingido" }),
    ...
    // Queries em paralelo
    const [receitasRes, despesasRes, receitasPrevRes, despesasPrevRes, projetosRes, folhaRes, leadsRes, despesasFuturas] = await Promise.all([
      adminClient.from("receitas").select("valor, status, projeto_id, categoria_id").eq("empresa_id", empresaId).gte("data_vencimento", startDate).lte("data_vencimento", endDate).is("deleted_at", null),
      adminClient.from("despesas").select("valor, status, projeto_id, categoria_id").eq("empresa_id", empresaId).gte("data_vencimento", startDate).lte("data_vencimento", endDate).is("deleted_at", null),
      adminClient.from("receitas").select("valor, status").eq("empresa_id", empresaId).gte("data_vencimento", prevStart).lte("data_vencimento", prevEnd).is("deleted_at", null),
      adminClient.from("despesas").select("valor, status").eq("empresa_id", empresaId).gte("data_vencimento", prevStart).lte("data_vencimento", prevEnd).is("deleted_at", null),
      adminClient.from("projetos").select("id, nome, status, valor_contrato").eq("empresa_id", empresaId).is("deleted_at", null).in("status", ["Planejamento", "Em andamento"]),
      adminClient.from("folha_pagamento").select("total_receber, status").eq("empresa_id", empresaId).eq("mes", targetMes).eq("ano", targetAno),
      adminClient.from("leads").select("id, status").eq("empresa_id", empresaId).gte("created_at", startDate).is("deleted_at", null),
