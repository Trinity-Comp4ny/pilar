...
import { formatCPF, formatCNPJ, formatPhone, formatAgency, formatBankAccount } from "@/lib/maskUtils";
...
export function PessoaFormDialog({ open, onOpenChange, editPessoa, onSaved }: PessoaFormDialogProps) {
  ...
  useEffect(() => {
    ...
    if (editPessoa) {
      ...
      const contas = Array.isArray(editPessoa.contas_bancarias) ? editPessoa.contas_bancarias : [];
      const pix = Array.isArray(editPessoa.chaves_pix) ? editPessoa.chaves_pix : [];
    ...
    try {
      ...
      if (isEditMode && editPessoa) {
        ...
        if (empresaError || !empresaId) {
          throw (
            empresaError ?? new Error("Não foi possível identificar sua empresa. Recarregue a página e tente de novo.")
          );
        }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto p-0">
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {isEditMode
                ...
                : "Nome, cargo e email já cadastram. Resto você completa quando precisar."}
            </DialogDescription>
          </DialogHeader>
        </div>
        ...
        <form onSubmit={(e) => e.preventDefault()} className="divide-y">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  className={cn(
                    form.formState.errors.primeiro_nome && "border-destructive focus-visible:ring-destructive"
                  )}
                />
          ...
          {/* Dados bancários — opcional, só importa quando a pessoa entrar na folha */}
          <div className="px-6">
            {canEditSensitive ? (
              ...
              <div className="py-4">
                <Label className="text-[10px] uppercase text-muted-foreground tracking-wider">Dados bancários</Label>
