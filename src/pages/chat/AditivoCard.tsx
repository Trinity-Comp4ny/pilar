...
export function AditivoCard({ index, draft, onConfirmar, onCancelar, onDesfazer }: Props) {
    ...
    setItens((prev) =>
      prev.map((it, idx) =>
        idx === i
          ? {
              ...it,
              [key]: key === "descricao" || key === "disciplina" ? value : value === "" ? undefined : Number(value),
            }
          : it
      )
    );
  ...
  const criar = async () => {
    ...
    setSalvando(true);
    try {
      await onConfirmar(draft.runId, "aditivo", {
  ...
  if (draft.status === "cancelado") {
    return (
      <div className="w-full max-w-lg rounded-2xl border border-border bg-muted/40 p-4 text-sm text-muted-foreground">
        Rascunho de aditivo descartado.
      </div>
    );
  }
      ...
      <div className="space-y-3 px-4 py-4">
        <div className="space-y-1.5">
          ...
          <Select value={projetoId} onValueChange={setProjetoId}>
            <SelectTrigger id={`aditivo-${index}-projeto`} className="h-9">
              <SelectValue placeholder={projetos.isLoading ? "Carregando…" : "Selecione…"} />
            </SelectTrigger>
            <SelectContent>
              {(projetos.data ?? []).map((p) => (
        ...
        <div className="space-y-1.5">
          ...
          <Input
            id={`aditivo-${index}-desc`}
            value={descricao}
            onChange={(e) => setDescricao(e.target.value)}
            disabled={salvando}
            className="h-9"
            placeholder="Resumo do aditivo"
          />
        </div>
        <div className="space-y-1.5">
          <Label htmlFor={`aditivo-${index}-just`} className="text-xs text-muted-foreground">
            Justificativa
          </Label>
          <Textarea
            id={`aditivo-${index}-just`}
            value={justificativa}
            onChange={(e) => setJustificativa(e.target.value)}
            rows={2}
            disabled={salvando}
            placeholder="Por que o escopo aumentou"
          />
        </div>
        ...
        <div className="space-y-2">
          ...
          {itens.map((it, i) => (
            <div key={i} className="grid grid-cols-[1fr_auto_auto_auto] items-center gap-2">
              <Input
                ...
                aria-label={`Item ${i + 1} descrição`}
                disabled={salvando}
                className="h-9"
              />
              <Input
                value={it.disciplina ?? ""}
                onChange={(e) => setItem(i, "disciplina", e.target.value)}
                placeholder="Disciplina"
                aria-label={`Item ${i + 1} disciplina`}
                disabled={salvando}
                className="h-9 w-28"
              />
              <Input
                type="number"
                value={it.custo ?? ""}
                onChange={(e) => setItem(i, "custo", e.target.value)}
                placeholder="Custo"
                aria-label={`Item ${i + 1} custo`}
                disabled={salvando}
                className="h-9 w-24"
              />
              <button
                ...
                className="flex h-9 w-9 items-center justify-center rounded-md text-muted-foreground hover:bg-muted hover:text-negative-strong"
              >
          ...
          <Button
            variant="ghost"
            size="sm"
            onClick={() => setItens((p) => [...p, { descricao: "" }])}
            disabled={salvando}
        ...
        <div className="rounded-lg border border-dashed border-border bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
          Custo dos itens: {formatCurrency(custoTotal)} · valor do aditivo (margem 30%):{" "}
          <span className="font-medium text-foreground">{formatCurrency(valorAditivo)}</span>
        </div>
      </div>
