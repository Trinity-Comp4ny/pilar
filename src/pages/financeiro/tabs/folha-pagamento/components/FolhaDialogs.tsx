...
export function CloseMonthDialog({ open, onOpenChange, selectedMonth, selectedYear, peopleCount, totalFolha, saving, allConfirmed, onConfirm }: CloseMonthDialogProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Você está prestes a fechar a folha de <strong>{getMonthLabel(selectedMonth)}/{selectedYear}</strong>.
            <br /><br />
            Isso irá gerar os registros financeiros para {peopleCount} pessoas, totalizando <strong>{formatCurrency(totalFolha)}</strong>.
            <br /><br />
      ...
      <DialogContent className="sm:max-w-md">
        ...
        {person && (
          <div className="space-y-3 text-sm">
            <div><span className="font-semibold">Colaborador: </span><span>{person.p_nome}</span></div>
            <div><span className="font-semibold">Cargo: </span><span>{person.p_cargo}</span></div>
            <div className="flex justify-between border-b pb-1"><span className="font-semibold">Salário Fixo: </span><span>{formatCurrency(person.p_salario_fixo)}</span></div>
            <div className="flex justify-between border-b pb-1"><span className="font-semibold">Variável: </span><span>{formatCurrency(person.v_variavel)}</span></div>
            <div className="flex justify-between text-lg pt-1"><span className="font-semibold">Total a Receber: </span><span className="font-bold text-green-600">{formatCurrency(person.v_total)}</span></div>
          </div>
        )}
        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)}>Cancelar</Button>
          <Button onClick={onConfirm} className="bg-accent-orange hover:bg-accent-orange/90">Confirmar Colaborador</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
export function DetailEditDialog({ open, onOpenChange, person, isEditing, editForm, onStartEditing, onCancelEditing, onSaveEditing, onEditFormChange }: DetailEditDialogProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {isEditing ? "Ajuste os valores manualmente. As alterações serão salvas apenas para este mês." : "Informações detalhadas do colaborador."}
          </DialogDescription>
        </DialogHeader>
        {person && (
          <div className="space-y-4 py-2">
            <div className="grid grid-cols-2 gap-4">
              ...
              <EditableField label="Salário Fixo" isEditing={isEditing} value={person.p_salario_fixo} editValue={editForm.p_salario_fixo} onChange={(v) => onEditFormChange("p_salario_fixo", v)} isCurrency />
              <EditableField label="Produtividade (m²)" isEditing={isEditing} value={person.soma_area} editValue={editForm.soma_area} onChange={(v) => onEditFormChange("soma_area", v)} suffix=" m²" />
              <EditableField label="Variável" isEditing={isEditing} value={person.v_variavel} editValue={editForm.v_variavel} onChange={(v) => onEditFormChange("v_variavel", v)} isCurrency />
              <EditableField label="Total" isEditing={isEditing} value={person.v_total} editValue={editForm.v_total} onChange={(v) => onEditFormChange("v_total", v)} isCurrency isBold />
            </div>
        ...
        <DialogFooter className="flex gap-2">
          {isEditing ? (
            <>
              <Button variant="outline" onClick={onCancelEditing}>Cancelar</Button>
              <Button onClick={onSaveEditing} className="bg-accent-orange hover:bg-accent-orange/90">Salvar Alterações</Button>
            </>
          ) : (
            <>
...
function EditableField({ label, isEditing, value, editValue, onChange, isCurrency, isBold, suffix }: {
  label: string; isEditing: boolean; value: number; editValue?: number;
  onChange: (v: number) => void; isCurrency?: boolean; isBold?: boolean; suffix?: string;
}) {
  return (
