 ...
 * Esta função pode ser usada em uma migration ou script de correção
 */
export const correctInvalidDates = (items: any[]): any[] => {
  return items.map(item => {
    const corrected = { ...item };
    
    // Se data_recebimento for inválida, define como null
    if (isInvalidDate(item.data_recebimento)) {
      corrected.data_recebimento = null;
    }
    
    // Se data_vencimento for inválida, define como null
    if (isInvalidDate(item.data_vencimento)) {
      corrected.data_vencimento = null;
    }
    
    // Se data_pagamento for inválida, define como null
    if (isInvalidDate(item.data_pagamento)) {
      corrected.data_pagamento = null;
    }
...
/**
 * Verifica se uma transação é da automação Bradesco
 * Baseado na presença de data_recebimento e ausência de data_vencimento
 */
export const isBradescoTransaction = (item: any): boolean => {
  return !!item.data_recebimento && !item.data_vencimento;
};

/**
 * Verifica se uma transação é manual
 * Baseado na presença de data_vencimento
 */
export const isManualTransaction = (item: any): boolean => {
